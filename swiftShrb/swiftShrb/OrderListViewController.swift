...
class OrderListViewController: UIViewController,UITableViewDataSource,UITableViewDelegate {
    ...
    var dataArray : NSMutableArray = [["storeLogoImageView":"辛巴克",
        "storeNameLabel":"辛巴克",
        "stateLabel":"交易成功",
        "orderImageView":"辛巴克",
        "moneyLabel":"105",
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        cell.moneyLabel.text = String(format: "实付金额:%@元", self.dataArray.objectAtIndex(indexPath.row)["moneyLabel"] as! String)

        let string =  self.dataArray.objectAtIndex(indexPath.row)["moneyLabel"] as! String
        let attrString : NSMutableAttributedString = NSMutableAttributedString(string: cell.moneyLabel.text!)
        
        attrString.addAttribute(NSForegroundColorAttributeName, value:shrbPink, range: NSMakeRange(5, string.characters.count+1))
    ...
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        self.navigationController?.pushViewController(OrderListDetialViewController(), animated: true)
    }
